...
# Purpose:
#   For simulations with true M = 150 and coverage range 20–1000:
...
out_pdf     <- "/rs/rs_grp_scaipgenetic/QuASAR2/tests/simulations/results/M_150_delta_coverage_bin_power_FDR_plots.pdf"
alpha <- 0.10
...
truth_all <- bind_rows(lapply(
  ...
  function(f) {
    ...
    if (is.null(p) || p$M != 150 || p$N_lo != 20 || p$N_hi != 1000)
      return(NULL)
