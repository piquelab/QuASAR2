...
fitQuasar2 <- function(dd, nbreaks = 20, M = 20, eps = 0.001) {
  ...
  res <- aux %>% mutate(res=map(data,fitBetaBinomialLogistic,~ Tr, eps=neweps))
...
##  res3 <- dd %>% group_by(identifier) %>% nest()
  ...
  res <- aux %>% mutate(res=map2(data,res, ~fitBetaBinomialLogistic(.x,model=~  Tr, b=.y$coeff,eps=neweps))) 
...
outPath <- "/nfs/rprscratch/wwwShare/carly/ASE_Plots/RogerEtOHonlyNoBatch/"
system(paste0("mkdir -p ",outPath))


png(file=paste0(outPath,"dex_case_bbl_qq.png"))
qq(resDex$pval)
dev.off()
...
p1 <- combDex %>% filter(padj<0.1) %>% ggplot(aes(meta_estimate.x,meta_estimate.y)) +
  geom_point() + geom_abline(slope=1,intercept = 0) +
  theme_bw()
p1

ggsave(paste0(outPath,"dex_case_bbl_scatter_meta.png"),p1)

p1 <- combDex %>% filter(padj<0.1) %>% ggplot(aes(meta_z.x,meta_z.y)) +
  geom_point() + geom_abline(slope=1,intercept = 0) +
  theme_bw()
p1
ggsave(paste0(outPath,"dex_case_bbl_scatter_metaZ.png"),p1)


png(file=paste0(outPath,"vitd_case_bbl_scatter_meta.png"))
qq(resVitD$pval)
dev.off()

combVitD <- inner_join(resVitD,myMetaCt,by="identifier") %>% inner_join(myMetaVitD,by="identifier")

p1 <- combVitD %>% filter(padj<0.1) %>% ggplot(aes(meta_estimate.x,meta_estimate.y)) +
  geom_point() + geom_abline(slope=1,intercept = 0) +
  theme_bw()
p1

ggsave(paste0(outPath,"VitD_case_bbl_scatter_meta.png"),p1)


png(file=paste0(outPath,"vita_case_bbl_scatter_meta.png"))
qq(resVitA$pval)
dev.off()

combVitA <- inner_join(resVitA,myMetaCt,by="identifier") %>% inner_join(myMetaVitA,by="identifier")

p1 <- combVitA %>% filter(padj<0.1) %>% ggplot(aes(meta_estimate.x,meta_estimate.y)) +
  geom_point() + geom_abline(slope=1,intercept = 0) +
  theme_bw()
p1

ggsave(paste0(outPath,"VitA_case_bbl_scatter_meta.png"),p1)
...
outPath <- "/rs/rs_grp_neanbit/RogerStuff/BetaBinomialLogisticResEtohOnlyNoBatch/"
system(paste0("mkdir -p ",outPath))

write.table(resDex,file=paste0(outPath,"case_bbl_Bitstarr_dex.txt"),quote=FALSE,row.names=FALSE,sep="\t")
...
write.table(resVitD,file=paste0(outPath,"case_bbl_Bitstarr_vitd.txt"),quote=FALSE,row.names=FALSE,sep="\t")
write.table(resVitA,file=paste0(outPath,"case_bbl_Bitstarr_vita.txt"),quote=FALSE,row.names=FALSE,sep="\t")
write.table(rescInt,file=paste0(outPath,"ase_bbl_Bitstarr.txt"),quote=FALSE,row.names=FALSE,sep="\t")
