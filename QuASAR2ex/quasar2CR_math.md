...
The per-SNP log-likelihood is $\ell_g = \sum_{i=1}^{n_g} \ell_{gi}$.

The full log-likelihood is $\ell(\boldsymbol{\theta}, \varepsilon) = \sum_g \ell_g$, where $\boldsymbol{\theta}$ collects all per-SNP coefficients and dispersions.
...
is downward-biased in $\psi_g$ when $p_g$ is non-negligible relative to $n_g$, because it treats $\hat{\boldsymbol{\beta}}_g$ as exact.
